...
using BusinesCardWebsite.Domain.Repositories.EntityFramework;
using BusinesCardWebsite.Service;
...
namespace BusinesCardWebsite
{
    public class Startup
    {
        ...
        {
            //это подключаю используемые конфигрурации
            Configuration.Bind("Project", new Config());

            //это  подключаю репозитории и менеджер для работы с ними
            services.AddTransient<ITextFieldsRepository, EFTextFieldsRepository>();
            services.AddTransient<IServiceItemsRepository, EFServiceItemsRepository>();
            services.AddTransient<DataManager>();

            //подключаю дбКонтекст БД
